    ...
    for(j=0;j<Y;j++)
    {
		...
		for(hy=-largeur;hy<=largeur;hy++)
		{
		    ...
		    for(hz=-largeur;hz<=largeur;hz++)
		    {
	        	zz = (k+hz)>0?(k+hz):0;
			zz = zz<(z-1)?zz:(z-1);
			coeff=exp(-((double) (abs(hx)+abs(hy)+abs(hz)))/beta);
 ...
 */
static int isaligned3( int x, int y, int z, grads angles, angles3 theta,
                      double prec )
{
  ...
  if( x < 0 || y < 0 || z < 0 || 
      x>=(int)angles->az->xsize || y>=(int)angles->az->ysize || z>=(int)angles->az->zsize||      x>=(int)angles->el->xsize || y>=(int)angles->el->ysize || z>=(int)angles->el->zsize)
  ...
  //fflush(stdout);
  if( az == NOTDEF ) return FALSE;
  if( el == NOTDEF ) return FALSE;  /* there is no need to call the function
                                      'double_equal' here because there is
                                      no risk of problems related to the
                                      comparison doubles, we are only
                                      interested in the exact NOTDEF value */
...
static double rect3_nfa(struct rect3 * rec, grads angles, double logNT,double *image,int N,int minreg)
{
  ...
  if( i->x >= 0 && i->y >= 0 && i->z >=0 &&
	...
	tester=1;
        ++pts; /* total number of pixels counter */
        if( isaligned3(i->x, i->y,i->z, angles, rec->theta, rec->prec) )
          ++alg; /* aligned points counter */
      }
  //if(tester==0){
                           ...
                           double * inputv)
{
 ...
 for(ang_mult=5;ang_mult<=9;ang_mult+=2)
 {
	 ...
	 for(xx=0;xx<xsize;xx+=1)
	 {
		 ...
		 {
				x_t        = isaligned3(xx,  yy+1, zz+1,  angles,angv,prec);
				x_tminus   = isaligned3(xx,  yy,   zz,    angles,angv,prec);
	 ...
	 for(yy=0;yy<ysize;yy+=1)
	 {
		 ...
		 for(xx=0;xx<xsize-1;xx+=1)
	         for(zz=0;zz<zsize-1;zz+=1)
		 {
				x_t      = isaligned3(xx+1,  yy,  zz+1, angles,angh,prec);
				x_tminus = isaligned3(xx,    yy,  zz,   angles,angh,prec);
	 ...
	 for(zz=0;zz<zsize;zz+=1)
	 {
		 ...
		 for(xx=0;xx<xsize-1;xx+=1)
	         for(yy=0;yy<ysize-1;yy+=1)
		 {
				x_t      = isaligned3(xx+1,  yy+1,  zz, angles,angd,prec);
				x_tminus = isaligned3(xx,    yy,    zz, angles,angd,prec);
...
static PyObject * clsd(PyObject * self, PyObject * args)
{
	...
	int Xin,Yin,Zin;
  	...
  	// parse arguments
  	if (!PyArg_ParseTuple(args, "OIIIOIIIO", &imagein, &X, &Y, &Z, &image0in, &X0, &Y0, &Z0, &inputvin)) {
    		return NULL;
  	} 
